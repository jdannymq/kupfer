...
import org.domain.sck.dto.DatosReservaSolicitudDTO;
...
@Name("informeMatrizResponsabilidadAction")
@Scope(ScopeType.CONVERSATION)
public class InformeMatrizResponsabilidadAction {
	...
	private List<DatosReservaSolicitudDTO> listaCumplidasGeneral;
	private List<DatosReservaSolicitudDTO> listaNoCumplidasGeneral;
	private List<DatosReservaSolicitudDTO> listaCumplidasMasNoCumplidasGeneral;

	private List<PlanillaDTO> listaPlanilla;
	private Jornada jornada;

	@Out(required = false, scope = ScopeType.EVENT)
	private List<PlanillaDTO> listaPlanillaFinal;
		...
		if (validarParametros()) {
			...
			if (listaUsuariosegursSeleccionados != null
				...
				for (Usuariosegur user : listaUsuariosegursSeleccionados) {
					StringBuffer cadena = new StringBuffer();
					cadena.append("'");
					cadena.append(user.getAlias());
					cadena.append("'");
					PorcentajeDTO objPorcentaje = getObtenerDatosParaTrabajoUsuario(
							cadena.toString(), fechaInicio, fechasFinal,
							tipoSolicitudes, canales, negocios, sucursales);
				...
				if (listaLcredTipoSolicitudsSeleccionados != null
						&& listaLcredTipoSolicitudsSeleccionados.size() > 0) {
					listaPorcentajeTipoSolicitudes = new ArrayList<PorcentajeDTO>(
							0);
						...
						for (PorcentajeDTO obj : listaPorcentajes) {
							if (obj != null) {
								for (LcredTipoSolicitud ti : listaLcredTipoSolicitudsSeleccionados) {
									PorcentajeDTO objPorTipo = getObtenerDatosParaTrabajoUsuarioAndTipoSolicitud(
											obj.getUsuario(), fechaInicio,
											fechasFinal, ti, canales, negocios,
											sucursales);
									if (obj.getListasProcentajeTipoSolicitudes() == null) {
										obj.setListasProcentajeTipoSolicitudes(new ArrayList<PorcentajeDTO>(
												0));
										obj.getListasProcentajeTipoSolicitudes()
												.add(objPorTipo);
									} else {
										obj.getListasProcentajeTipoSolicitudes()
												.add(objPorTipo);
	...
	public void limpiarParametos() {
		...
		this.setListaCanalesSeleccionada(new ArrayList<Canal>());
		this.setListaNegociosSeleccionados(new ArrayList<ConceptosNegocio>());
		this.setListaLcredTipoSolicitudsSeleccionados(new ArrayList<LcredTipoSolicitud>());
		this.setListaSucursalsSeleccionados(new ArrayList<Sucursal>());
		this.setListaUsuariosegursSeleccionados(new ArrayList<Usuariosegur>());
		this.setListaTodasSolicitudesDto(new ArrayList<SolicitudDTO>());
		this.setTsListaTodasSolicitudes(null);
		this.setTotalSolicitudes(new Long(0));
		this.setCumplidas(new Long(0));
		this.setNocumplidas(new Long(0));
		this.setListaPorcentajeTipoSolicitudes(null);
		this.setListaPorcentajes(null);
	...
	public void sacarUsuariosSegunSucursalesSeleccionadas() {
		...
		StringBuffer cadena = new StringBuffer();
		/*
		 * try { if (listaSucursalsSeleccionados != null &&
		 * listaSucursalsSeleccionados.size() != 0) { int cantidad =
		 * listaSucursalsSeleccionados.size(); for (Sucursal s :
		 * listaSucursalsSeleccionados) { if ((contador + 1) == cantidad) {
		 * cadena.append("'"); cadena.append(s.getCodigo()); cadena.append("'");
		 * } else { cadena.append("'"); cadena.append(s.getCodigo());
		 * cadena.append("'"); cadena.append(","); contador++; } }
		 * log.debug("cadena #0", cadena.toString());
		 * 
		 * if (cadena != null && !"".equals(cadena.toString())) {
		 * listaUsuariosegurs = scoringService
		 * .getListaUsuariosSegunMatrizResponsabilidades(cadena .toString()); }
		 * } else { listaUsuariosegurs = new ArrayList<Usuariosegur>(); } }
	...
	public void getObtenerDatosParaTrabajoMatriz(String usuarios,
			String fechaInicio, String fechaTermino, String tipoSolicitudes,
		...
		HitosDTO sacarFechasAnterior = null;
		...
		try {
			/* listado de feriados */
			listaFeriados = scoringService.getConsultaFeriadosSegunRangoFechas(
					fechaInicio, fechaTermino);
			/* listado de tiempos de la solicitudes */
			List<HitosDTO> listaHitos = scoringService
					.getConsultaMatrizResponsabilidades(usuarios, fechaInicio,
							fechaTermino, tipoSolicitudes, canales, negocios,
							sucursales);

			/* Realizar el trabajo de evaluacion */
			if (listaHitos != null) {
				listaCumplidasGeneral = new ArrayList<DatosReservaSolicitudDTO>();
				listaNoCumplidasGeneral = new ArrayList<DatosReservaSolicitudDTO>();
				listaCumplidasMasNoCumplidasGeneral = new ArrayList<DatosReservaSolicitudDTO>();

				for (HitosDTO obj : listaHitos) {
					if (idSolicitud == 0) {
						log.info("solicitud " + obj.getIdSolicitud());
						...
						if (obj.getIdSolicitud() == idSolicitud) {
							...
							totalSolicitudes++;
							if (idSolicitud == 32131 || idSolicitud == 32272
									|| idSolicitud == 32514
									|| idSolicitud == 32664) {
							...
							if (fechaInicial.equals(fechaFinal)) {
								...
								int resto = 0;
								int segundo = Integer.parseInt(h2[2])
										- Integer.parseInt(h1[2]);
								if (segundo < 0) {
									resto = -1;
									segundo = 60 - segundo;
								}
								int minuto = (Integer.parseInt(h2[1]) - Integer
										.parseInt(h1[1])) - resto;
								resto = 0;
								if (minuto < 0) {
									minuto = 60 - minuto;
								...
								hora = (Integer.parseInt(h2[0]) - Integer
										.parseInt(h1[0])) - resto;

								System.out.println("Diferencia o Respuesta= "
										+ hora + ":" + minuto + ":" + segundo);

							} else {
								if ("".equals(fechaFinal) || fechaFinal == null) {
									fechaFinal = enSession.getFechaHitos();
								}
								...
								int FMAYOR = Integer.parseInt(anoM + "" + mesM
										+ "" + diaM);
								int FMENOR = Integer.parseInt(anom + "" + mesm
										+ "" + diam);
								...
								if (FMAYOR >= FMENOR) {
									...
									System.out
											.println("Numero de dias contados -> "
													+ (cont - 1));
									System.out
											.println("Con el dia de inicio incluido serian -> "
													+ cont);
									/* calcular las hora entre los dias. */

								} else {
									System.out
											.println("Error....!!La fecha Menor supera a la fecha Mayor");
								}
							...
							if ("1C".equals(enSession.getTipoSolicitud())
									|| "1N".equals(enSession.getTipoSolicitud())) {
								horaTotales = 0;
								if (lineaCredito.longValue() < 0) {
									System.out.println("Con Sobre Giro");
									tiempoMontoType = TiempoMontoType.MONTOS;

									if ("1C".equals(enSession
											.getTipoSolicitud())) {
										tipo = scoringService
												.obtenerObjetoLcredTipoSolicitud("11");
									} else {
										tipo = scoringService
												.obtenerObjetoLcredTipoSolicitud("12");
									}
									medicion = scoringService
											.obtenerObjetoMedicion(2);

									if (enSession.getCanal().KX
											.equals(TipoCuentasKupferType.KX)) {
										canal = scoringService
												.obtenerCanalForCodigo(1);
									} else if (enSession.getCanal().MX
											.equals(TipoCuentasKupferType.MX)) {
										canal = scoringService
												.obtenerCanalForCodigo(3);
									} else if (enSession.getCanal().GC
											.equals(TipoCuentasKupferType.GC)) {
										canal = scoringService
												.obtenerCanalForCodigo(2);
									}

									List<CondicionTipoSolicitudRango> lista = scoringService
											.getCondicionTipoSolicitudRango(
													tipo, canal,
													tiempoMontoType, medicion);
									if (fechaInicial.equals(fechaFinal)) {
										if (lista != null && lista.size() == 1) {
											CondicionTipoSolicitudRango nuevo = lista
													.get(0);
											if (hora <= nuevo.getHoras()
													.longValue()) {
												cumplidas++;
												DatosReservaSolicitudDTO o = new DatosReservaSolicitudDTO();
												o.setIdSolicitud(idSolicitud);
												listaCumplidasGeneral.add(o);
												listaCumplidasMasNoCumplidasGeneral
														.add(o);
												contada = true;
											} else {
												contada = true;
											...
											if (lineaCredito.longValue() < 0) {/*
												...
												for (CondicionTipoSolicitudRango nuevo : lista) {
													if (nuevo
															.getMedicionCanalRango()
															.getValorInicial()
															.longValue() > 60
															&& nuevo.getMedicionCanalRango()
																	.getValorFinal()
																	.longValue() > 60) {
														paraCaclulo = nuevo;
														break;
										...
										if (cont > 1) {
											for (int i = 0; i < cont; i++) {
												...
												DateFormat fechaAux = new SimpleDateFormat(
														"dd-MM-yyyy");
												convertido = fechaAux
														.format(cal.getTime());
												System.out.println(convertido);
												if (listaFeriados
														.contains(convertido) == false) {
									...
									.equals(enSession.getTipoSolicitud())) {
								horaTotales = 0;
								System.out.println("Con Sobre Giro");
								tiempoMontoType = TiempoMontoType.MONTOS;

								tipo = scoringService
										.obtenerObjetoLcredTipoSolicitud("23");
								...
								List<CondicionTipoSolicitudRango> lista = scoringService
										.getCondicionTipoSolicitudRango(tipo,
												canal, tiempoMontoType,
												medicion);
								if (fechaInicial.equals(fechaFinal)) {
									if (lista != null && lista.size() == 1) {
										CondicionTipoSolicitudRango nuevo = lista
										...
										if (lineaCredito != null) {
											if (lineaCredito.longValue() < 0) {
												lineaCredito = new BigDecimal(
														lineaCredito
																.longValue()
																* -1);
											}
											long valorReal = lineaCredito
													.longValue();
									...
									if (cont > 1) {
										for (int i = 0; i < cont; i++) {
											...
											DateFormat fechaAux = new SimpleDateFormat(
													"dd-MM-yyyy");
											convertido = fechaAux.format(cal
													.getTime());
											System.out.println(convertido);
											if (listaFeriados
													.contains(convertido) == false) {
								...
								if ("2I".equals(enSession.getTipoSolicitud())) {
									tipo = scoringService
											.obtenerObjetoLcredTipoSolicitud("21");
								} else if ("2N".equals(enSession
										.getTipoSolicitud())) {

									tipo = scoringService
											.obtenerObjetoLcredTipoSolicitud("22");
								} else if ("3R".equals(enSession
										.getTipoSolicitud())
										|| "3P".equals(enSession
												.getTipoSolicitud())
										|| "3RP".equals(enSession
												.getTipoSolicitud())) {

									tipo = scoringService
											.obtenerObjetoLcredTipoSolicitud("31");
								} else if ("4B".equals(enSession
										.getTipoSolicitud())
										|| "4D".equals(enSession
												.getTipoSolicitud())) {

									tipo = scoringService
											.obtenerObjetoLcredTipoSolicitud("41");
								} else if ("4DM".equals(enSession
										.getTipoSolicitud())) {

									tipo = scoringService
											.obtenerObjetoLcredTipoSolicitud("43");
								} else if ("4PR".equals(enSession
										.getTipoSolicitud())) {

									tipo = scoringService
											.obtenerObjetoLcredTipoSolicitud("44");
								}
								medicion = scoringService
										.obtenerObjetoMedicion(3);

								List<CondicionTipoSolicitudRango> lista = scoringService
										.getCondicionTipoSolicitudRango(tipo,
												null, tiempoMontoType, medicion);
								if (fechaInicial.equals(fechaFinal)) {
									...
									if (lista != null && lista.size() == 1) {
										...
										CondicionTipoSolicitudRango paraCaclulo = null;
										lineaCredito = new BigDecimal(
												lineaCredito.longValue() * -1);
										for (CondicionTipoSolicitudRango nuevo : lista) {
											if (nuevo.getMedicionCanalRango()
													.getValorInicial()
													.longValue() >= 0
													&& nuevo.getMedicionCanalRango()
															.getValorFinal()
			...
			e.printStackTrace();
			log.error(
					"Error, no se puede realizar la la verificacion de existecia de negocio ",
					e.getMessage());
		}

	}

	public PorcentajeDTO getObtenerDatosParaTrabajoUsuario(String usuario,
			String fechaInicio, String fechaTermino, String tipoSolicitudes,
			String canales, String negocios, String sucursales) {
		...
		try {
			...
			/* listado de tiempos de la solicitudes */
			List<HitosDTO> listaHitos = scoringService
					.getConsultaMatrizResponsabilidades(usuario, fechaInicio,
							fechaTermino, tipoSolicitudes, canales, negocios,
							sucursales);
			...
			if (porcentajeFinal != null) {
				...
				if (dividir != 0) {
					double dividir2 = (dividir / porcentajeFinal
							.getTotalSolicitudesIndividual());
			...
			e.printStackTrace();
			log.error("Error, se cae al verificar lka informacion de trabajo del usuario #0 y el error es el siguiente #1",
	...
	public PorcentajeDTO getObtenerDatosParaTrabajoUsuarioAndTipoSolicitud(
			String usuario, String fechaInicio, String fechaTermino,
			LcredTipoSolicitud tipoSolicitudes, String canales,
			String negocios, String sucursales) {
		PorcentajeDTO porcentajeFinal = new PorcentajeDTO();
		long idSolicitud = 0;
		String hora1 = ""; // 03:15:20
		String hora2 = ""; // 04:20:45
		...
		try {
			...
			switch (Integer.parseInt(tipoSolicitudes.getCodTipoSolicitud()
					.trim())) {
			case 11:
				tipoSol = "'1N'";
				break;
			...
			List<HitosDTO> listaHitos = scoringService
					.getConsultaMatrizResponsabilidades(usuario, fechaInicio,
							fechaTermino, tipoSol, canales, negocios,
							sucursales);
			...
			e.printStackTrace();
			log.error(
					"Error, se cae al verificar lka informacion de trabajo del usuario #0 y el error es el siguiente #1",
					usuario, e.getMessage());
		}
	...
	public void generarGrillaParaExcel(String opcion) {
		try {
			if (opcion != null && "TODAS".equals(opcion)) {
				log.debug("cantidad  de datos: "
						+ listaCumplidasMasNoCumplidasGeneral.size());
				if (listaCumplidasMasNoCumplidasGeneral.size() > 0) {
					listaPlanilla = new ArrayList<PlanillaDTO>(0);
					for (DatosReservaSolicitudDTO drs : listaCumplidasMasNoCumplidasGeneral) {
						PlanillaDTO p = getDatosPlanilla(drs);
						if (p != null) {
							listaPlanilla.add(p);
						}
					}
				}
			} else if (opcion != null && "CUMPLIDAS".equals(opcion)) {
				log.debug("cantidad  de datos: " + listaCumplidasGeneral.size());
				if (listaCumplidasGeneral.size() > 0) {
					listaPlanilla = new ArrayList<PlanillaDTO>(0);
					for (DatosReservaSolicitudDTO drs : listaCumplidasGeneral) {
						PlanillaDTO p = getDatosPlanilla(drs);
						if (p != null) {
							listaPlanilla.add(p);
						}
				...
				log.debug("cantidad  de datos: "
						+ listaNoCumplidasGeneral.size());
				if (listaNoCumplidasGeneral.size() > 0) {
					listaPlanilla = new ArrayList<PlanillaDTO>(0);
					for (DatosReservaSolicitudDTO drs : listaNoCumplidasGeneral) {
						PlanillaDTO p = getDatosPlanilla(drs);
						if (p != null) {
							listaPlanilla.add(p);
						}
			...
			e.printStackTrace();
			log.debug("error al sacar los datos la grilla.");
		}
	}

	public PlanillaDTO getDatosPlanilla(DatosReservaSolicitudDTO datosReserva) {
		List<String> listaFeriados = scoringService
				.getConsultaFeriadosSegunRangoFechas("01-01-2010", "31-12-2014"); // yyyyMMdd
		List<String> listaHoras = new ArrayList<String>(0);
		PlanillaDTO planillaDTO = null;
		String inicioJornada = null;
		String terminoJornada = null;
		List<PlanillaHitosDTO> listaHitosNuevos = null;
		HitosDTO hitoAnterior = null;
		try {
			if (datosReserva != null) {
				if (datosReserva.getIdSolicitud() == 35738) {
					log.debug("solicitud #0", datosReserva.getIdSolicitud());
				}
				List<HitosDTO> listaHitos = scoringService
						.getObtenerDatosHitosPorSolicitud(datosReserva
								.getIdSolicitud());
				if (listaHitos != null && listaHitos.size() > 0) {

					inicioJornada = getHoraJornada("Inicial");
					terminoJornada = getHoraJornada("Termino");
					listaHitosNuevos = new ArrayList<PlanillaHitosDTO>(0);
					planillaDTO = new PlanillaDTO();
					HitosDTO hcabecera = listaHitos.get(0);
					planillaDTO.setIdSolictud(String.valueOf(hcabecera
							.getIdSolicitud()));
					planillaDTO.setFechaEmision(hcabecera.getFechaCreacion());
					planillaDTO.setRut(hcabecera.getRut());
					planillaDTO.setRazonSocial(hcabecera.getNombre());
					planillaDTO.setMonto(String.valueOf(hcabecera.getMonto()));
					planillaDTO.setTipoSolicitud(hcabecera
							.getDescripcionTipoSolicitud());
					planillaDTO.setAprobadores(hcabecera.getAprobadores());
					planillaDTO.setConceptosInvolucrados(hcabecera
							.getConceptos());
					for (HitosDTO h : listaHitos) {
						boolean pasa = true;
						if ("DE".equals(h.getEstado().trim())) {
							pasa = false;
						} else if ("ES".equals(h.getEstado().trim())) {
							pasa = false;
						} else if ("SA".equals(h.getEstado().trim())) {
							pasa = false;
						} else if ("B".equals(h.getEstado().trim())) {
							pasa = false;
						} else if ("AE".equals(h.getEstado().trim())) {
							pasa = false;
						} else if ("AP".equals(h.getEstado().trim())) {
							pasa = false;
						}

						PlanillaHitosDTO planillaHitosDTO = new PlanillaHitosDTO();
						planillaHitosDTO.setFecha(h.getFechaHitos());
						planillaHitosDTO.setHora(h.getHoraHitos());
						planillaHitosDTO.setEstado(h.getEstado());
						planillaHitosDTO.setDescripcionEstado(h
								.getDescripcionEstado());
						if ("I".equals(h.getEstado().trim())) {
							planillaHitosDTO.setTiempo(h.getTiempoInicial());
							if (pasa) {
								listaHoras.add(h.getTiempoInicial());
							}
							listaHitosNuevos.add(planillaHitosDTO);
						} else {
							if ((hitoAnterior.getFechaHitos().trim()).equals(h
									.getFechaHitos().trim())) {
								String diferencia = calculoDeHoras(
										hitoAnterior.getHoraHitos(),
										h.getHoraHitos());
								System.out.println("Diferencia o Respuesta= "
										+ diferencia);
								planillaHitosDTO.setTiempo(diferencia);
								if (pasa) {
									listaHoras.add(diferencia);
								}
							} else {
								String diferencia = "";
								int cont = 0;
								...
								SimpleDateFormat formatoDelTexto = new SimpleDateFormat(
										"dd-MM-yyyy");
								Date fecha = formatoDelTexto.parse(hitoAnterior
										.getFechaHitos());
								cal.setTime(fecha);
								String convertido = "";
								for (int i = 0; i < cont; i++) {
									...
									System.out.println(convertido);
									if (!convertido.equals(h.getFechaHitos())) {
										if (!convertido.equals(hitoAnterior
												.getFechaHitos())) {
											if (listaFeriados
													.contains(convertido) == false) {
												horaTotales = horaTotales + 9;
								...
								/*
								 * calcular el rango hitos anterior al final de
								 * la jornada
								 */
								String primeraDiferencia = calculoDeHoras(
										hitoAnterior.getHoraHitos(),
										terminoJornada);

								/*
								 * calculo de ranfo de siguientes hito con el
								 * inicio de la jornada
								 */
								String segundaDiferencia = calculoDeHoras(
										inicioJornada, h.getHoraHitos());

								if (primeraDiferencia != null
										&& segundaDiferencia != null) {
									List<String> listaNuevaFechas = new ArrayList<String>(
											0);
									listaNuevaFechas.add(primeraDiferencia);
									listaNuevaFechas.add(segundaDiferencia);
									diferencia = sumaDeHoras(listaNuevaFechas,
											horaTotales);
								}
						...
						/*
						 * else{
						 * if(!"00:00:00".equals(h.getTiempoHitoInicial().trim())){
						 * PlanillaHitosDTO planillaHitosDTO = new
						 * PlanillaHitosDTO();
						 * planillaHitosDTO.setFecha(h.getFechaHitos());
						 * planillaHitosDTO.setHora(h.getHoraHitos());
						 * planillaHitosDTO.setEstado(h.getEstado());
						 * planillaHitosDTO
						 * .setDescripcionEstado(h.getDescripcionEstado());
						 * planillaHitosDTO.setTiempo(h.getTiempoHitoInicial());
						 * listaHitosNuevos.add(planillaHitosDTO); }
						 * hitoAnterior = h; }
						 */
					}

					PlanillaHitosDTO planillaHitosTerminoDTO = new PlanillaHitosDTO();
					planillaHitosTerminoDTO
							.setDescripcionEstado("Tiempo Total");
					String tiempoTotal = sumaDeHoras(listaHoras, 0);
					planillaHitosTerminoDTO.setTiempo(tiempoTotal);
					listaHitosNuevos.add(planillaHitosTerminoDTO);
					planillaDTO.setListaHitos(listaHitosNuevos);
				}
			}

		} catch (Exception e) {
			log.error("error al sacar los tiempos de la solicitud :  #0",
					datosReserva.getIdSolicitud());
		}

		return planillaDTO;
	}

	public String getHoraJornada(String opcion) {
		try {
			if ("Inicial".equals(opcion)) {
				SimpleDateFormat sdf = new SimpleDateFormat("HH:MM:ss");
				return sdf.format(jornada.getHoraInicial());
			} else {
				SimpleDateFormat sdf = new SimpleDateFormat("HH:MM:ss");
				return sdf.format(jornada.getHoraFinal());
			}
		} catch (Exception e) {
			log.error("jornada #0", e.getMessage());
		}
		return null;
	}

	public String calculoDeHoras(String horaInicial, String horaTermino) {
		String diferencia = "";
		Double hora = new Double(0);
		Double horasP = new Double(0);
		Double minutosP = new Double(0);
		String[] h1 = horaInicial.split(":");
		String[] h2 = horaTermino.split(":");
		int resto = 0;
		...
		int minuto = (Integer.parseInt(h2[1]) - Integer.parseInt(h1[1]))
				- resto;
		resto = 0;
		if (minuto < 0) {
			minuto = 60 - minuto;
		...
		hora = (double) (Integer.parseInt(h2[0]) - Integer.parseInt(h1[0]))
				- resto;
		System.out.println("Diferencia o Respuesta= " + hora + ":" + minuto
				+ ":" + segundo);

		if (segundo > 60) {
			minutosP = (double) (segundo / 60);
			minuto += minutosP;
			segundo = segundo - (int) (minutosP.doubleValue() * 60);
		}

		if (minuto > 60) {
			horasP = (double) (minuto / 60);
			hora += horasP;
			minuto = minuto - (int) (horasP * 60);
		}

		if (hora.longValue() > 9) {
			diferencia += String.valueOf(hora.longValue()) + ":";
		} else {
			diferencia += "0" + String.valueOf(hora.longValue()) + ":";
		}
		if (minuto > 9) {
			diferencia += String.valueOf(minuto) + ":";
		} else {
			diferencia += "0" + String.valueOf(minuto) + ":";
		}
		if (segundo > 9) {
			diferencia += String.valueOf(segundo);
		} else {
			diferencia += "0" + String.valueOf(segundo);
		}
	...
	public String sumaDeHoras(List<String> listaHoras, int horasExtras) {
		String horaFinal = "";
		try {
			Double horas = new Double(0);
			Double minuto = new Double(0);
			Double segundo = new Double(0);
			int horasP = 0;
			int minutosP = 0;
			if (listaHoras != null) {
				for (String h : listaHoras) {
					String[] hora = h.split(":");
					String horaa = hora[0].replace("-", "");
					horas += Double.parseDouble(horaa);
					String minutoo = hora[1].replace("-", "");
					minuto += Double.parseDouble(minutoo);
					String segundoo = hora[2].replace("-", "");
					segundo += Double.parseDouble(segundoo);

				}
			}

			if (segundo > 60) {
				minutosP = (int) (segundo / 60);
				minuto += minutosP;
				segundo = segundo - (minutosP * 60);
			}

			if (minuto > 60) {
				horasP = (int) (minuto / 60);
				horas += horasP;
				minuto = minuto - (horasP * 60);
			}
			...
			if (horas > 9) {
				horaFinal += String.valueOf(horas.longValue()) + ":";
			} else {
				horaFinal += "0" + String.valueOf(horas.longValue()) + ":";
			}
			if (minuto > 9) {
				horaFinal += String.valueOf(minuto.longValue()) + ":";
			} else {
				horaFinal += "0" + String.valueOf(minuto.longValue()) + ":";
			}
			if (segundo > 9) {
				horaFinal += String.valueOf(segundo.longValue());
			} else {
				horaFinal += "0" + String.valueOf(segundo.longValue());
			}
			System.out.println("Diferencia o Respuesta= " + horas.longValue()
					+ ":" + minuto.longValue() + ":" + segundo.longValue());

		} catch (Exception e) {
			log.error("no se puedo sumar todas las horas, revisar el metodo",
					e.getMessage());
		}
	...
	public void setListaPlanilla(List<PlanillaDTO> listaPlanilla) {
	...
	public void setListaPlanillaFinal(List<PlanillaDTO> listaPlanillaFinal) {
		this.listaPlanillaFinal = listaPlanillaFinal;
	}
