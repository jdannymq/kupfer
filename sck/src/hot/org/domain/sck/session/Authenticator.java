...
import conexion.LoginController;
...
//		    				credentials.getPassword()); 
	    		...
	    		if(connect != null){
					...
					FacesMessages.instance().add(org.jboss.seam.international.StatusMessage.Severity.ERROR,"El sistema no logra conectar a SAP, revisar conectividad...!!!!");
					connect = null;
					return false;
	    		}
        	}
        	...
        	try{
        		LoginController conecionLdap = new LoginController();
       			int valorParaMensaje = conecionLdap.authenticateSession(credentials.getUsername().trim(), credentials.getPassword());
        		switch (valorParaMensaje) {
				case 2:
					System.out.println("Conexion deshabilitada");
					FacesMessages.instance().add(Severity.ERROR,"Conexion deshabilitada. #0",credentials.getUsername().trim());
					return false;	
				case 3:
					System.out.println("Autentificacion invalida");
					FacesMessages.instance().add(Severity.ERROR,"Autentificacion invalida. #0",credentials.getUsername().trim());
					return false;	
				case 4:
					System.out.println("clave de administrador no corresponde.");
					FacesMessages.instance().add(Severity.ERROR,"clave de administrador no corresponde. #0",credentials.getUsername().trim());
					return false;						
				default:
					break;
				}
